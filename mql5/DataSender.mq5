//+------------------------------------------------------------------+
//| DataSender.mq5 — Sends H1 candle data to LongEntry server       |
//| Runs on each of the 14 charts independently                      |
//| Trigger: Friday, 1 hour before that symbol's market close        |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   Print("[DataSender] Initialized for ", _Symbol,
...
void OnTick()
  {
   ...
   // Run 1 hour before session close — use a simple heuristic:
   // trigger after 75% of the trading day has passed
   // A more precise approach would check SymbolInfoSessionTrade
   if(dt.hour < 12)
      return;
   ...
   if(UploadCandles())
     {
      ...
      Print("[DataSender] Upload successful for ", _Symbol);
     }
  }


//+------------------------------------------------------------------+
//| Upload candle data to server                                      |
//+------------------------------------------------------------------+
bool UploadCandles()
  {
   ...
   if(g_lastUploadTime == 0)
     {
      ...
      Print("[DataSender] First upload — fetching ", InitialYears, " years of H1 data");
     }
   else
     {
   ...
   if(copied <= 0)
     {
      Print("[DataSender] ERROR: CopyRates returned ", copied);
      return false;
     }
   ...
   for(int attempt = 1; attempt <= MaxRetries; attempt++)
     {
      ...
      if(res == 200)
        {
         ...
         Print("[DataSender] Chunk ", chunkNum, "/", totalChunks, " sent OK: ", response);
         return true;
        }
      ...
      if(attempt < MaxRetries)
        {
         Print("[DataSender] Retrying in ", RetryDelaySec, " seconds...");
         Sleep(RetryDelaySec * 1000);
        }
     }

   Print("[DataSender] ERROR: All ", MaxRetries, " attempts failed for chunk ", chunkNum);
   return false;
  }
...
void OnDeinit(const int reason)
  {
   Print("[DataSender] Deinitialized for ", _Symbol);
  }
